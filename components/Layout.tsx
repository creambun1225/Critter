  ...
  useEffect(() => {
    ...
    const unsubPersonal = onSnapshot(personalQ, (snap) => {
      ...
      snap.docs.forEach((d: any) => {
        const data = d.data();
        if (!(data.readBy || []).includes(currentUser.uid)) personalUnread++;
      });
    ...
    if (currentUser.admin) {
      ...
      unsubReport = onSnapshot(reportQ, (snap) => {
        ...
        snap.docs.forEach((d: any) => {
          const data = d.data();
          if (!(data.readBy || []).includes(currentUser.uid)) reportUnread++;
        });
        setNotificationCount(personalUnread + reportUnread);
      });
    }
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      const stopWords = new Set([
        "の","に","は","を","が","で","と","た","て","も","な","い","る","し","れ","さ","ん","だ",
        "か","ら","や","よ","ね","わ","け","ど","あ","う","え","お","http","https","www",
        "rt","the","a","an","is","in","it","of","to","and","or","for",
      ]);
      snap.docs.forEach((d: any) => {
        ...
        words.forEach((word: string) => { tagCount[word] = (tagCount[word] || 0) + 1; });
      });
