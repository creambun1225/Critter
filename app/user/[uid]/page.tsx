      ...
      try {
        ...
        if (snap.exists()) {
          ...
          const followerDetails = await Promise.all(
            followersList.map(async (fid: string) => {
              const fSnap = await getDoc(doc(db, "users", fid));
              return fSnap.exists() ? { uid: fid, ...fSnap.data() } : null;
            })
          );
          setFollowers(followerDetails.filter(Boolean));

          const followingDetails = await Promise.all(
            followingList.map(async (fid: string) => {
              const fSnap = await getDoc(doc(db, "users", fid));
              return fSnap.exists() ? { uid: fid, ...fSnap.data() } : null;
            })
          );
  ...
  return (
    <Layout currentUser={currentUser}>
        ...
        <div className="px-4 pb-4">
          <div className="flex items-end justify-between mb-4 -mt-16">
            ...
            {!isOwn && (
              <div className="flex gap-2">
                {currentUser?.admin && (
                  <button onClick={banUser} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-full font-bold transition">
                    🚫 BAN
                  </button>
                )}
                ...
                <div className="relative">
                  <button onClick={() => setMenuOpen(!menuOpen)} className="text-2xl px-4 py-2 hover:bg-zinc-900 rounded-full transition">
                    ⋯
                  </button>
                  {menuOpen && (
                    <>
                      <div className="fixed inset-0 z-40" onClick={() => setMenuOpen(false)} />
                      <div className="absolute right-0 top-12 bg-black border border-zinc-700 rounded-2xl overflow-hidden w-48 z-50 shadow-2xl">
                        ...
                        <button onClick={() => { toggleMute(); setMenuOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-zinc-900 text-sm">
                          {isMuted ? "✓ ミュート中" : "🔕 ミュート"}
                        </button>
                      </div>
                    </>
          ...
          <div>
            ...
            <div className="flex gap-4 mt-3 text-sm text-zinc-500">
              <div>
                <span className="text-white font-bold">{followers.length}</span> フォロワー
              </div>
              <div>
                <span className="text-white font-bold">{following.length}</span> フォロー中
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="border-b border-zinc-800 flex">
        <button className="flex-1 py-4 px-4 border-b-2 border-white font-bold text-white hover:bg-zinc-900/50 transition">
          投稿
        </button>
      </div>
