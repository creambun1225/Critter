...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setBannedUsers(snap.docs.map((d) => ({ uid: d.id, ...d.data() })));
    });
    return () => unsub();
  }, [userData?.admin]);
  ...
  const toggleTheme = () => {
    ...
    setIsDark(next);
    try {
      if (next) { localStorage.setItem("critter_theme", "dark"); document.documentElement.classList.remove("light"); }
      else { localStorage.setItem("critter_theme", "light"); document.documentElement.classList.add("light"); }
    } catch {}
  };

  const logout = async () => { await signOut(auth); location.href = "/login"; };

  const changePassword = async () => {
    if (!newPassword) return;
    try {
      if (auth.currentUser) { await updatePassword(auth.currentUser, newPassword); alert("変更しました"); setNewPassword(""); }
    } catch { alert("再ログインしてください"); }
  };
      ...
      <div className="p-6 text-white">
        ...
        <div className="bg-zinc-900 rounded-2xl p-5 mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-zinc-500 text-sm mt-1">{isDark ? "現在ダークモードです" : "現在ライトモードです"}</div>
            </div>
            <button onClick={toggleTheme}
              className={`relative w-14 h-7 rounded-full transition-colors ${isDark ? "bg-blue-500" : "bg-zinc-600"}`}>
              <span className={`absolute top-0.5 left-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform ${isDark ? "translate-x-7" : "translate-x-0"}`} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 rounded-2xl p-5 mb-6">
          ...
          <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)}
            placeholder="新しいパスワード" className="w-full bg-black border border-zinc-700 rounded-xl p-3 mb-4" />
        ...
        <div className="bg-zinc-900 rounded-2xl p-5 mb-6">
          ...
          <input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)}
            placeholder="管理者パスワード" className="w-full bg-black border border-zinc-700 rounded-xl p-3 mb-4" />
        ...
        {isAdmin && (
          <>
            <Link href="/admin/reports" className="w-full block bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/40 rounded-xl p-4 text-left text-yellow-400 font-bold mb-4">
              📋 報告一覧（管理者）
            </Link>

            {/* BANリスト */}
            <button onClick={() => setShowBanList(!showBanList)}
            ...
            {showBanList && (
              <div className="bg-zinc-900 rounded-2xl overflow-hidden mb-4">
                {bannedUsers.length === 0 ? (
                  <p className="text-center text-zinc-500 py-6 text-sm">BANされたユーザーはいません</p>
                ) : (
                  bannedUsers.map((user) => (
                    <div key={user.uid} className="p-4 border-b border-zinc-800 last:border-b-0">
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <img src={user.icon || "/default.png"} className="w-8 h-8 rounded-full object-cover bg-zinc-700 shrink-0" />
                            <div>
                          ...
                          <p className="text-zinc-400 text-xs mt-2 leading-relaxed">
                            <span className="text-zinc-600">理由: </span>{user.banReason || "記載なし"}
                          </p>
                        </div>
