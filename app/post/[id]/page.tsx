...
export default function PostDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    const q = query(
      collection(db, "posts", id as string, "replies"),
  ...
  const submitReply = async () => {
    if (!currentUser || !canSubmit || !id) return;
    setLoading(true);
    try {
      ...
      <div className="p-4 border-b border-zinc-800">
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* 本文（大きめ） */}
            <p className="mt-3 text-[18px] whitespace-pre-wrap break-words leading-relaxed text-white">
              {post.text}
            </p>
            ...
            <div className="flex gap-6 mt-3 border-t border-zinc-800 pt-3 text-sm text-zinc-400 flex-wrap">
              <span>
                <span className="font-bold text-white">{post.reposts || 0}</span>{" "}
                リポスト
              </span>
              <span>
                <span className="font-bold text-white">{post.likes || 0}</span>{" "}
                いいね
              </span>
              <span>
                <span className="font-bold text-white">{post.replies || 0}</span>{" "}
                リプライ
              </span>
              <span>
                <span className="font-bold text-white">{post.bookmarks || 0}</span>{" "}
                ブックマーク
              </span>
            </div>
          </div>
        ...
        <div className="p-4 border-b border-zinc-800 flex gap-3">
          ...
          <div className="flex-1">
            <textarea
              ...
              placeholder={`@${post.username} へリプライ...`}
              rows={3}
